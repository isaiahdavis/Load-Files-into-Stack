...
var g_StackScriptFolderPath = app.path + "/" + localize( "$$$/ScriptingSupport/InstalledScripts=Presets/Scripts" ) + "/" + localize( "$$$/private/LoadStack/StackScriptOnly=Stack Scripts Only/" );
...
// loadLayers routines
loadLayers = new ImageStackCreator( localize( "$$$/AdobePlugin/Shared/LoadStack/Process/Name=Load Layers" ), localize( '$$$/AdobePlugin/Shared/LoadStack/Auto/untitled=Untitled' ) );
...
loadLayers.createSmartObject = true; // If true, option to create smart object is checked.
...
loadLayers.customDialogSetup = function ( w ) {
	...
	if ( !app.featureEnabled( localize( "$$$/private/ExtendedImageStackCreation=ImageStack Creation" ) ) ) w.findControl( '_createSO' ).hide( );
}
...
loadLayers.stackLayers = function ( ) {
	...
	if ( this.createSmartObject ) {
		// load each layer into smartObject
		for (var a = 0; a < stackDoc.layers.length; a++) {
			app.activeDocument.activeLayer = stackDoc.layers[a];
...
loadLayers.doInteractiveLoad = function ( ) {
	...
	if ( this.stackElements ) this.stackLayers( );
	else return 'cancel';
}
